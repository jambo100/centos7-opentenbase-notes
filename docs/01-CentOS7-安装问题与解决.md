...
>
> 配这个最直接的好处：以后 `ssh root@192.168.105.10` 直接连，不用每次去 VMware 窗口里敲命令。

### 4. 为什么必须固定 IP

**因为有一堆东西把这个地址写死了，DHCP 一换号就全断。**

| 位置 | 写死的值 |
|---|---|
| 集群配置 `opentenbase_config.ini` | 4 个节点的 host 全是 `192.168.105.10` |
| 宿主机快捷方式 `连接OpenTenBase.bat` | ping 和 ssh 都指向这个 IP |
| 常用别名 `otb-sql` | `psql -h 192.168.105.10 -p 11003` |
| 体检脚本、各处复用的命令 | 同上 |

**最关键的是第一行。** 普通单机「IP 变了只是我连不上」，在这里是另一个量级：OpenTenBase 是 4 个节点互连的集群，DN 要按这个地址去连 CN 的 forward 端口、连 GTM 领全局事务 ID —— **IP 对这个集群来说是拓扑信息，不是「入口地址」**。地址一变，`otb-status` 显示节点全在，日志里却全是连不上。

至于 DHCP 本身：VMware 通常按 MAC 分配地址，**看着挺稳定，但那是在碰运气** —— 租约到期、虚拟网络编辑器改过设置、VMware 升级重置网络，都可能让它换号。固定下来就没有这个不确定性。

> 顺带澄清一个容易混的点：这个静态 IP **和你连哪个 WiFi 无关**。`192.168.105.0/24` 是 VMware 在你电脑内部虚拟出来的网段，外面根本不存在，所以换到别的网络它照样是 `192.168.105.10`。
